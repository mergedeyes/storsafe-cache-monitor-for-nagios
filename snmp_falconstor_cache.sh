...
for object in "${objects_perc[@]}"; do
    ...
    if [[ $counter -eq 1 ]]; then
        ...
        if [[ $(echo "$output_perc_save > $critical_threshold" | bc -l) -eq 1 ]]; then
            output_avai_perc_info="SERVICE STATUS: CRITICAL - Used Available-Capacity: $output"
        elif [[ $(echo "$output_perc_save > $warning_threshold" | bc -l) -eq 1 ]]; then
            output_avai_perc_info="SERVICE STATUS: WARNING - Used Available-Capacity: $output"
        else
...
for object in "${objects_gb[@]}"; do
    ...
    if [[ $counter -eq 1 ]]; then
      ...
      output_used_gb_perf="used_cache_gb=${output_gb}GB"
        ...
        if [[ $(echo "$output_perc_save > $critical_threshold" | bc -l) -eq 1 ]]; then
            output_avai_gb_info="SERVICE STATUS: CRITICAL - Used Available-Cache-Capacity: $output"
        elif [[ $(echo "$output_perc_save > $warning_threshold" | bc -l) -eq 1 ]]; then
            output_avai_gb_info="SERVICE STATUS: WARNING - Available-Cache-Capacity: $output"
        else
            output_avai_gb_info="SERVICE STATUS: OK - Used Available-Cache-Capacity: $output"
        fi
